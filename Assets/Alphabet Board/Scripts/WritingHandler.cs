...
public class WritingHandler : MonoBehaviour
{
    ...
    public bool clickStarted;//uses with mouse input drawings,when drawing clickStarted
    ...
    IEnumerator Start()
    {
        _MainCamera = CommonData.prefabs.gameobjectLookup["CameraLetter"].GetComponent<Camera>();
    ...
    void Update()
    {
        ...
        RaycastHit2D hit2d = Physics2D.Raycast(_MainCamera.ScreenToWorldPoint(Input.mousePosition), Vector2.zero);//raycast hid c

        if (hit2d.collider != null)
        {
            if (Input.GetMouseButtonDown(0))
            {
                TouchLetterHandle(hit2d.collider.gameObject, true, _MainCamera.ScreenToWorldPoint(Input.mousePosition));//touch for letter move(drawing);
                clickStarted = true;
            }
            else if (clickStarted)
            {
                TouchLetterHandle(hit2d.collider.gameObject, false, _MainCamera.ScreenToWorldPoint(Input.mousePosition));//touch for letter move(drawing);
            }
        }
